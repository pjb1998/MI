...
void vPowerChipTask(void *p)
{
    ...
    {       
        ...
        vRechargeStandUpdate();//充电座工作信息更新
        ...
        }else
        {
            ...
            if(g_tRobotState.Robotstate.PreviousDirection == zCharging)//如果上次状态为充电中
            {
                ...
                g_tRobotState.Robotstate.PreviousDirection = zCharging;	//继续
            }
        }
        vTaskDelay(100);
    }
...
{
    if(g_usAnalogSwitch[adcRechargeStand] > 2000)//充电座电流adc检测	,根据adc检测到的电流判断是否处于充电状态
