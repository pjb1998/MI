...
* Return         : 电机占空比 450~50
*******************************************************************************/
 u16 usSpeed2Cycle(s16 loc_Speed)
{
...
{
    int16_t PidTemp;	//pid参数，临时值
    PidTemp = sSpeed_L_PID( g_tLeftWheel.SoftSpeed, g_tLeftWheel.RealSpeed );//PID计算下一时刻左轮速度
    if( PidTemp < 0 )//左轮倒退
    {                         
    ...
    PidTemp = sSpeed_R_PID( g_tRightWheel.SoftSpeed, g_tRightWheel.RealSpeed );//PID计算下一时刻左轮速度
