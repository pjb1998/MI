...
u16 g_AdcOriginalValue[ADC_TRANSFORM_NUM][ADC_CHANNEL_NUM];//adc原始数据<--DMA采集
...
void vAllAdcPinInit(void)
{
    ...
    ADC_DeInit(ADC1);
		ADC_InitStructure.ADC_Mode = ADC_Mode_Independent;	//ADC转换模式—独立模式(还有个双ADC模式)
    ...
    DMA_DeInit(DMA1_Channel1);	//将DMAyChannelx寄存器的初始化为其默认值
    ...
    DMA_Cmd(DMA1_Channel1, ENABLE);//使能(ENABLE)或者失能(DISABLE)DMA外设
...
{
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;	//切换上下文，中断中切换必须手动置为pdFALSE避免不必要的切换，任务切换到高优先级任务的调度为自动切换,详情见：https://blog.csdn.net/qq_25370227/article/details/86553879
    
    if( DMA_GetITStatus( DMA1_IT_TC1 )== SET )//DMA传输完成 
    {
        DMA_ClearITPendingBit( DMA1_IT_TC1);	//清除中断标志位,进中断首先要做的事情
        ADC_Cmd( ADC1, DISABLE );
        xTaskNotifyFromISR( HandleADCDataProcessing , 1 , eIncrement , &xHigherPriorityTaskWoken );//发送消息给adc处理任务->切换中断上下文（手动）
    }          
}
